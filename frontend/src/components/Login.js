...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send only email and password to loginUser API
      ...
      if (response.data?.user?.id) {
        ...
        // Save companyId to sessionStorage
        sessionStorage.setItem("companyId", companyId);

        console.log(response.data);

        // Check if the companyId exists, if so, navigate to the dashboard
        if (companyId) {
          // User is associated with the company, navigate to the dashboard with companyId
